...
const cardPrefixes = ['Two', 'Three', 'Four', 'Five', 'Six', 'Seven', 'Eight', 'Nine', 'Ten', 'Jack', 'Queen', 'King', 'Ace'];
...
function getRndmNum(min, max) { // min and max included 
...
function getTrickWinner(gameState){
   ...
   console.log('');
   console.log(`lead: ${leadCard} of ${leadSuit}`);
   console.log(`trump: ${trumpCard} of ${trumpSuit}`);

   let winnerId = gameState.leadPlayerId; //default winner
   let winnerCard = gameState.players[gameState.leadPlayerIndex].cardPlayedThisTurn;
   ...
   gameState.players.forEach(player => {     
      ...
      console.log(`${player.name} played a ${cardSuit} of ${player.cardPlayedThisTurn}`);

      //check if card is highest lead if trump has not been played
      if(highestTrumpPlayed === -1){
         if(cardSuit === leadSuit && player.cardPlayedThisTurn >= highestLeadPlayed) {
      ...
      if(cardSuit === trumpSuit && player.cardPlayedThisTurn >= highestTrumpPlayed) {
         winnerCard = player.cardPlayedThisTurn;
         winnerId = player.socketId;
   ...
   addMessage(gameState, `${player.name} won with ${getCardName(player.cardPlayedThisTurn)}`)
   player.tricksWon += 1;
   return winnerId;
}
...
function scorePoints(gameState){
   gameState.players.forEach(player => {
      player.points += player.tricksBet === player.tricksWon ? (10 + player.tricksBet) : player.tricksWon;
  });
}
